...
const Near = () => {
  ...
  useEffect(() => {
    ...
    const savedAddress = localStorage.getItem('near_address');
    const savedNetwork = localStorage.getItem('near_network');
    const savedConnected = localStorage.getItem('near_connected') === 'true';

    // Check if NEAR wallet is available
    const checkWallet = async () => {
      if (window.nearApiJs && window.nearApiJs.wallet) {
        setProvider(window.nearApiJs.wallet);
      }
    };
    checkWallet();
    ...
    try {
      if (provider) {
        ...
        if (accounts.length > 0) {
          ...
          setBalance('0.00 NEAR');
          setConnectionStatus('✓ Wallet connected successfully!');

          // Save to localStorage
          localStorage.setItem('near_address', accountId);
          localStorage.setItem('near_network', selectedNetwork);
          localStorage.setItem('near_connected', 'true');
        }
      } else {
  ...
  const disconnectWallet = async () => {
    ...
    // Clear localStorage
    localStorage.removeItem('near_address');
    localStorage.removeItem('near_network');
    localStorage.removeItem('near_connected');
  };

  const switchNetwork = (selectedNetwork) => {
    setNetwork(selectedNetwork);
    localStorage.setItem('near_network', selectedNetwork);
    const networkName = selectedNetwork === 'mainnet' ? 'NEAR Mainnet' : 'NEAR Testnet';
    alert(`Please switch to ${networkName} in your wallet.`);
  };

  const getNetworkName = () => {
    return network === 'mainnet' ? 'NEAR Mainnet' : 'NEAR Testnet';
  };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-black">
      ...
      <main className="flex-grow flex flex-col items-center justify-center p-10">
        ...
        <p className="text-gray-400 mb-8">Scalable Blockchain Infrastructure</p>

        {isConnected && address ? (
          <>
            <div className="bg-slate-700 bg-opacity-50 backdrop-blur shadow-lg rounded-lg p-6 mb-6 w-full max-w-md border border-green-500">
              <h2 className="text-xl font-semibold mb-4 text-green-400">Wallet Connected</h2>
              <div className="mb-4">
              ...
              <div className="mb-4">
                ...
                <p className="text-sm font-mono break-all bg-slate-800 p-2 rounded text-green-400">
                  {address}
                </p>
              </div>
              <div className="mb-4">
          ...
          <>
            <p className="text-lg mb-6 text-gray-300">Connect your NEAR wallet to get started</p>

            <div className="flex flex-col sm:flex-row gap-4 mb-8">
              <button
                onClick={() => connectWallet('mainnet')}
                className="px-6 py-3 bg-green-600 hover:bg-green-700 text-white rounded-lg font-semibold transition shadow-lg"
              >
                Connect to NEAR Mainnet
              </button>
              <button
                onClick={() => connectWallet('testnet')}
                className="px-6 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-semibold transition shadow-lg"
              >
